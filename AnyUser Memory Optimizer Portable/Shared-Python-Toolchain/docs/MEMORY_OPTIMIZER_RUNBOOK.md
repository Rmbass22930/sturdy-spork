...
## Paths
- Source root: `J:\_shared_toolchains\Shared-Python-Toolchain`
- CLI module: `memory_optimizer.cli`
- Setup executable: `J:\_shared_toolchains\Shared-Python-Toolchain\dist\HyperVSetup.exe`
...
```powershell
Set-Location 'J:\_shared_toolchains\Shared-Python-Toolchain'
...
python -m memory_optimizer.cli hyperv-setup SecurityLab --skip-enable-hyperv --script-path 'J:\_shared_toolchains\Shared-Python-Toolchain\temp\StartMemoryOptimizer.ps1'
```
The second command should return JSON with `startup_mode`.

## 5. Remove setup (cleanup)
```powershell
python -m memory_optimizer.cli hyperv-uninstall --script-path 'J:\_shared_toolchains\Shared-Python-Toolchain\temp\StartMemoryOptimizer.ps1'
```
...
```
Output executable:
- `J:\_shared_toolchains\Shared-Python-Toolchain\dist\HyperVSetup.exe`

## 7. Smoke test executable
```powershell
cmd /c ""J:\_shared_toolchains\Shared-Python-Toolchain\dist\HyperVSetup.exe" --help"
```
