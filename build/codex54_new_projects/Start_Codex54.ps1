...
function Ensure-CodexLogin {
  ...
  if ($statusCode -eq 0 -and $statusText -notmatch "Not logged in") {
    return $true
  }
  ...
  if ($postStatusCode -eq 0 -and $postStatusText -notmatch "Not logged in") {
    return $true
  }
...
function cx {
  ...
  $CliArgs = @($CliArgs)
  if (-not ($CliArgs.Count -gt 0 -and $CliArgs[0] -eq "login")) {
  ...
  $runArgs = @("-m", $Model, "-C", "$PSScriptRoot") + $CliArgs
