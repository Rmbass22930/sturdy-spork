...
function Invoke-CodexLauncher {
  ...
  if ($env:CODEX_FORCE_PROJECT_HOME -eq '1') {
    $codexHome = Join-Path $WorkingDirectory '.codex-home'
  ...
  if (-not $codexCommand) {
    Write-Error "Codex CLI not found on PATH. Install/update Codex CLI, then run $LauncherName again."
    exit 127
  }
  ...
  function Test-LocalCodexAuthExists {
    ...
    $paths += (Join-Path $WorkingDirectory '.codex-home\auth.json')
    ...
    try {
      ...
      try {
        ...
        foreach ($line in $output) {
          ...
          if ($textLine -match '(?i)^WARNING:\s+failed to clean up stale arg0 temp dirs:') {
            continue
          }
          $line
        }
  ...
  $commonArgs = @('-m', $Model, '-C', $WorkingDirectory)
  if ($EnableSearch) {
    $commonArgs = @('--search') + $commonArgs
  }
  ...
  if ($CliArgs.Count -gt 0 -and $CliArgs[0] -eq 'exec') {
    ...
    $runArgs = $commonArgs + @('exec', '--skip-git-repo-check') + $execTail
    $code = Invoke-CodexRun -CodexArgs $runArgs -ExecMode
    if ($code -ne 0) { exit $code }
    return
  }

  $runArgs = @('--no-alt-screen') + $commonArgs + $CliArgs
